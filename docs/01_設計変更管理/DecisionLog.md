...
**第1期における犬猫販売価格（`pets.price`）は全ロールで税抜表示とし、WithTama 側で税込計算を行わない**

- **決定内容:**
  - `pets.price` = **税抜**販売価格（[Decision No.141](#decision-no141) の DB 意味を維持）
  - ブリーダー入力 = **税抜**価格。DB 保存も **税抜**のまま
  - 購入希望者向け（PU-01 / PU-02 / BY-03）= **税抜**表示（例: `250,000円（税抜）`）
  - ブリーダー向け（BR-10 / BR-11）= **税抜**表示・入力
  - 管理者向け（AD-11）= **税抜**表示（税込参考表示は第1期では追加しない）
  - WithTama は犬猫販売価格について **税込計算を行わない**（税率保持なし、税計算 utility なし、税率 env 追加なし）
  - 犬猫販売価格について **Stripe は使用しない**（Checkout / Tax Rate 等）
  - **Migration 不要**

- **No.140 との関係:** No.140 の「DB 正本 = 税抜」は **維持**。No.140 の「購入希望者向けは表示レイヤーで税込総額」は **`pets.price` に限り本 Decision が優先**（上書き）。No.140 本文は改訂しない

- **No.141 との関係:** No.141 の `pets.price` = 税抜 は **完全維持**。No.141 の「購入希望者向け税込表示レイヤー」条項は **`pets.price` に限り本 Decision が優先**

- **No.142 との関係:** **変更なし**（開発データ再投入 + 以降税抜統一）

- **No.146 との関係:** Stripe 月額会費の Tax Rate / コード固定禁止は **維持**。犬猫 `pets.price` の公開表示については **税計算を行わない**ため、No.146 の犬猫向け表示レイヤー税計算は第1期では **不要**

- **Stripe 月額会費との分離:** WithTama → ブリーダー月額 5,000 円（税抜）は Stripe Checkout + Dashboard Tax Rate。**ブリーダー ↔ 購入希望者**の犬猫参考価格は `pets.price` のみ（Stripe 非使用）

- **法務・税務:** 購入希望者向けに税抜価格のみを掲載することの適法性について **税理士・弁護士または専門家への確認が必要**。確認結果により仕様変更する可能性がある

- **専門家確認 NG 時:** その時点で **再 Decision** とし、必要に応じて税込表示等へ方針転換する（本 Decision を無理に維持しない）

- **理由:** 犬猫代金は WithTama の決済対象外。月額会費（Stripe）と掲載参考価格の責務を分離し、第1期実装を最小化する（[影響調査](../09_開発履歴/2026-09-09_犬猫価格税抜表示方針_Decision変更影響調査.md)）

- **影響範囲:** `formatPetPrice` / `formatAdminPetPrice`、PU-01/02、BY-03、BR-08/10/11、AD-11、`pet-draft-form-fields.tsx`

- **決定日:** 2026-09-09

- **参照:** [Decision No.140](#decision-no140) / [No.141](#decision-no141) / [No.142](#decision-no142) / [No.146](#decision-no146) / [2026-09-09 税抜表示統一 実装報告](../09_開発履歴/2026-09-09_犬猫価格_税抜表示統一_実装報告.md)

---

## Decision No.154

**第1期の法務公開ページ URL・同意導線・運営会社情報を確定する（文案は専門家レビュー後に差し替え可能）**

- **法務公開 URL（第1期）:**
  - `/terms` — 利用規約（共通 + ブリーダー関連 + 購入希望者関連を **1 URL 内** に含む）
  - `/privacy` — プライバシーポリシー
  - `/legal` — 特定商取引法に基づく表記（**主対象: WithTama → ブリーダー月額会費**）
  - `/company` — 運営会社情報
  - `/tokusho` `/about` `/guidelines` 等の **別 URL は第1期で新設しない**（ガイドライン等は `/terms` 内）

- **運営会社情報（確定）:**
  - 株式会社システムサイエンス / 代表者 磯野 幸治
  - 〒860-0016 熊本県熊本市中央区山崎町66番地
  - 096-322-6311（9:00～17:30）/ withtama@ssci.co.jp / http://www.ssci.co.jp/

- **会員登録同意:** signup 時に **利用規約 + プライバシーポリシーへの明示 checkbox 同意必須**。ブリーダーは同一 signup で「ブリーダー関連条項を含む」表示
- **同意履歴 DB:** 第1期では **`terms_accepted_at` 等の専用 DB 列・Migration は追加しない**

- **月額会費表記:** ブリーダー向けユーザー表示は **`月額 5,000円（税込）`** に統一（`BILLING_PLAN_PRICE_LABEL`、BR-13、`/legal`、`/terms`、Stripe 申込前補足）。犬猫販売価格の `…円（税抜）` とは **別商品・別表記**

- **犬猫価格:** [Decision No.153](#decision-no153) 維持 — `pets.price` 税抜整数、表示 `…円（税抜）`、税計算なし

- **文案:** 弁護士・税理士・管轄自治体レビュー **前のドラフト** を掲載。コンポーネント分割により **専門家レビュー後の差し替え可能**

- **Footer:** 公開 layout + auth layout に共通 `SiteFooter`（利用規約 / PP / 特商法 / 運営会社）

- **Stripe Checkout:** 設定変更は本 Decision の主目的外。申込直前画面に料金・自動更新・解約条件・規約リンクを **補足表示**

- **影響範囲:** `src/features/legal/**`, `(public)/terms|privacy|legal|company`, `(auth)/layout`, signup, BR-13 billing view, 問い合わせ・見学・BR-09 書類提出の PP 導線
