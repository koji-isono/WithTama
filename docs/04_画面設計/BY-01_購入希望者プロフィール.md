...
第1期方針として、**問い合わせ送信には `profile_completed = true` を必要条件** とする。

| 項目                   | 本設計での位置づけ                                             |
| ---------------------- | -------------------------------------------------------------- |
| 問い合わせ UI          | **未実装・本設計対象外**                                       |
| 問い合わせ API / RLS   | **変更しない**                                                 |
| profile_completed 強制 | 問い合わせ実装時にアプリ層のみか RLS でも保証するか **未決定** |

PU-02 公開犬猫詳細の問い合わせ CTA は引き続き第1期スコープ外（[PU-02](./PU-02_公開犬猫詳細.md)）。
...
問い合わせ成立後に、どの情報を・いつ・誰に・どの方法で開示するかは **問い合わせ機能設計時に Decision として決定** する。
...
| 2   | 問い合わせ時の `profile_completed` をアプリ層のみで保証するか RLS でも保証するか |
