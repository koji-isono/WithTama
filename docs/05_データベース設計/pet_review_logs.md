...
- 差戻し時: `pets.status` を `under_review` → `draft` と同時に `action = returned` と差戻し理由を INSERT する（Decision No.96）。
- 承認時: `pets.status` を `under_review` → `published` と同時に `action = approved` を INSERT する。
