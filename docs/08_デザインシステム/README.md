...
### 第1期ヘッダー・ナビ（Decision No.150）

| 要素                    | 第1期                                                                       |
| ----------------------- | --------------------------------------------------------------------------- |
| ヘッダー: 画面名        | 表示                                                                        |
| ヘッダー: ブリーダー名  | Supabase 実データ                                                           |
| ヘッダー: ログアウト    | `LogoutButton` → `signOut()` → `/login`                                     |
| ヘッダー: 通知ベル      | **非表示**（将来: サイト内通知センター / Popover / Resend）                 |
| PC サイドバー           | ダッシュボード / 犬猫管理 / 見学管理 / 問い合わせ / プロフィール / 月額会費 |
| `/breeder/settings` nav | **第1期非表示**（route 未実装）                                             |

問い合わせ未読等は BR-12 等の各機能画面で確認する（ヘッダー bell 集約なし）。
