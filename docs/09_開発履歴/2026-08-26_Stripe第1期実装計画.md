...
| 種別    | パス                                                             |
| ------- | ---------------------------------------------------------------- |
| Route   | `src/app/api/billing/checkout/route.ts` — `POST` のみ            |
| feature | `src/features/billing/` — handler, gate, session params, URLs 等 |
| test    | `scripts/test-stripe-step3-checkout.mts` / `-live.mts`           |
| npm     | `test:stripe-step3-checkout` / `test:stripe-step3-checkout-live` |

### API

| 項目       | 内容                                                                                  |
| ---------- | ------------------------------------------------------------------------------------- |
| Path       | `POST /api/billing/checkout`                                                          |
| 認証       | ログイン済み `users.role = breeder` のみ（admin / buyer → 403）                       |
| 審査       | `review_status = approved` 必須                                                       |
| 会員状態   | `pending` / `canceled` → Checkout 可。`active` / `suspended` → 403                    |
| レスポンス | `{ url }` のみ（Secret / Session ID 非返却）                                          |
| Price      | `STRIPE_BREEDER_PRICE_ID`（サーバー env のみ）                                        |
| metadata   | `metadata.breeder_id` + `subscription_data.metadata.breeder_id`                       |
| Customer   | 既存 `stripe_customer_id` あれば `customer`。なければ `customer_email`（DB 書込なし） |
| URL        | `NEXT_PUBLIC_APP_URL` + `/breeder/dashboard?checkout=success\|cancel`                 |
| 税         | 手動 Tax Rate（`STRIPE_BREEDER_TAX_RATE_ID`）。`automatic_tax` 不使用                 |

### DB 変更

なし（Checkout 完了・`stripe_customer_id` 紐付けは Step 4 Webhook）

### セキュリティ

- line_items はサーバー組み立て。env の Price ID のみ
- クライアントから `price_id` / `amount` / `breeder_id` / success・cancel URL 不可
- `membership_status` 更新・課金列 UPDATE なし（Step 1 trigger 維持）
- Stripe 生エラー・Secret をレスポンス / ログに出さない

### テスト

- 静的: `npm run test:stripe-step3-checkout`（37 PASS）
- Test Mode 実通信: `npm run test:stripe-step3-checkout-live`
- ブラウザ E2E: Test Mode 決済・Subscription 作成確認済（2026-08-28）

### 完了条件

- [x] Checkout Session 作成 API 実装
- [x] `metadata.breeder_id` が Session / subscription_data に付与
- [x] Step 3 静的テスト PASS
- [x] Test Mode Checkout が完了画面まで到達（ブラウザ E2E 確認済）
- [x] commit / push
...
| Step 3 live テスト       | **PASS** — Test Mode Session 作成・手動 Tax Rate 検証                   |
| ブラウザ E2E             | **PASS** — 5,500 円決済成功・Subscription 作成（Stripe Dashboard 確認） |
| Step 2 / Step 1 回帰     | commit 前最終確認 PASS                                                  |
| lint / typecheck / build | commit 前最終確認 PASS                                                  |
...
| 項目                          | 内容                                                                    |
| ----------------------------- | ----------------------------------------------------------------------- |
| `suspended` の Checkout       | No.144 上は再契約可能だが Subscription 状態未確認のため Step 3 では 403 |
| 二重クリック / 未完了 Session | 固定 Idempotency Key は使わない。DB 列追加なし。Step 4 以降で検討       |
