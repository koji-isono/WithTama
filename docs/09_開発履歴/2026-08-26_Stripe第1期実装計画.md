...
- [ ] Migration 適用済み（リモート / ローカル）
- [ ] trigger テスト PASS
- [ ] 設計書 `breeders.md` 列定義更新
...
| lib  | `src/lib/stripe/client.ts`（server-only） |
| lib  | `src/lib/stripe/config.ts` — env 検証     |
...
- Stripe Test Mode で Price retrieve（手動 or スクリプト）

### 完了条件

- [ ] `npm install stripe` 済み
- [ ] server client が Test Mode で Price を retrieve できる
- [ ] Secret が Client Bundle に含まれない（grep / build 確認）
